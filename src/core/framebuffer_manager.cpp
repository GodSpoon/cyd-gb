...
bool FramebufferManager::init() {
    ...
    // Try to allocate double buffers
    front_buffer = (uint16_t*)heap_caps_aligned_alloc(
        32, FB_SIZE, MALLOC_CAP_DMA | MALLOC_CAP_8BIT
    );
    back_buffer = (uint16_t*)heap_caps_aligned_alloc(
        32, FB_SIZE, MALLOC_CAP_DMA | MALLOC_CAP_8BIT
    );
    ...
    if (!front_buffer || !back_buffer) {
        ...
        if (front_buffer) free(front_buffer);
        if (back_buffer) free(back_buffer);
        front_buffer = (uint16_t*)heap_caps_aligned_alloc(32, FB_SIZE, MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
        back_buffer = nullptr;
        if (!front_buffer) {
            front_buffer = (uint16_t*)malloc(FB_SIZE);
        }
